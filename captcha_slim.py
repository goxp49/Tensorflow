...
def inference():
    ...
    net = slim.conv2d(x_image, 32, [3, 3], scope='conv1')  # shape of net is [N,114,450,32]
    net = slim.conv2d(x_image, 32, [3, 3], scope='conv1')  # shape of net is [N,114,450,32]
    net = slim.max_pool2d(net, [2, 2], scope='conv1')  # shape of net is [N,57,225,32]

    ### 第二层卷积操作 ###
    print('######### 2 ###########')
    net = slim.conv2d(net, 64, [3, 3], scope='conv2')
    net = slim.conv2d(net, 64, [3, 3], scope='conv2')
    net = slim.max_pool2d(net, [2, 2], scope='conv2')  # shape of net is [N,29,113,64]

    ### 第三层卷积操作 ###
    print('######### 3 ###########')
    net = slim.conv2d(net, 128, [3, 3], scope='conv3')
    net = slim.conv2d(net, 128, [3, 3], scope='conv3')
    net = slim.max_pool2d(net, [2, 2], scope='conv3')  # shape of net is [N,15,57,128]

    ### 第四层全连接操作 ###
    print('######### 4 ###########')
    net = tf.reshape(net, [-1, 15 * 57 * 128])
