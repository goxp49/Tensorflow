...
parser.add_argument('--backend', choices=backends, default=cv.dnn.DNN_BACKEND_DEFAULT, type=int,
                    help="Choose one of computation backends: "
                         ...
                         "Intel's Deep Learning Inference Engine (https://software.intel.com/openvino-toolkit), "
                         "OpenCV implementation")
parser.add_argument('--target', choices=targets, default=cv.dnn.DNN_TARGET_CPU, type=int,
                    help='Choose one of target computation devices: '
                         'CPU target (by default), '
                         'OpenCL, '
                         'OpenCL fp16 (half-float precision), '
                         'VPU')
...
# Load a network
net = cv.dnn.readNetFromTensorflow(os.path.join(OUTPUT_PATH, 'output_graph.pb'), os.path.join(OUTPUT_PATH, 'protobuf.pbtxt'))
